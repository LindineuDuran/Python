...
from app.infrastructure.ocr.tesseract_config import TesseractConfig
...
class OCRService:
    def extract_text(self, wx_image, rotation=0):
        try:
            ...
            config = TesseractConfig.get_config("--oem 3 --psm 11 -l por+eng")

            return pytesseract.image_to_string(pil_img, config=config)

        except Exception as e:
            logger.exception("Erro inesperado em OCR")
    ...
    def extract_region(self, wx_image, rect, rotation=0):
        try:
            ...
            if roi is None or roi.size[0] == 0 or roi.size[1] == 0:
                logger.warning("ROI inválida:", rect)
                return ""
